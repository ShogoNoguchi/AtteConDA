...
# ここから挿入してください（models/local_adapter.py の import 付近）

from models.patch_adaptation import PatchPonderPAM, PAMConfig
...
class LocalResBlock(nn.Module):
    ...
    def forward(self, x, emb, local_conditions):
        if not self.use_checkpoint:
            return self._forward(x, emb, local_conditions)
...
class FeatureExtractor(nn.Module):
    def __init__(self, local_channels, inject_channels, 
        ...
        # ===== PAM追加（デフォルトOFF）=====
        use_patch_adaptation: bool = False,
        pam_cfg: dict | None = None,           
                 
                 ):
        super().__init__()

        self.use_patch_adaptation = bool(use_patch_adaptation)
        ...
        # ===== PAM増築（use_patch_adaptation=True のときだけ有効）=====
        self.patch_adapt: PatchPonderPAM | None = None
        if self.use_patch_adaptation:
            if pam_cfg is None:
                pam_cfg = {}

            # PAMConfig のデフォルトは Uni-ControlNet/あなたのDataset に合わせてある
            cfg = PAMConfig(
                edge_slice=tuple(pam_cfg.get("edge_slice", (0, 3))),
                depth_slice=tuple(pam_cfg.get("depth_slice", (15, 18))),
                seg_slice=tuple(pam_cfg.get("seg_slice", (18, 21))),
                model_channels=int(pam_cfg.get("model_channels", 320)),
                context_dim=int(pam_cfg.get("context_dim", 768)),
                gating_dim=int(pam_cfg.get("gating_dim", inject_channels[0])),  # 192を想定
                num_heads=int(pam_cfg.get("num_heads", 8)),
                attn_dropout=float(pam_cfg.get("attn_dropout", 0.0)),
                temperature=float(pam_cfg.get("temperature", 1.0)),
                log_every=int(pam_cfg.get("log_every", 500)),
                enhancer_enable=bool(pam_cfg.get("enhancer_enable", False)),
                enhancer_n_blocks=int(pam_cfg.get("enhancer_n_blocks", 2)),
                enhancer_groups=int(pam_cfg.get("enhancer_groups", 32)),
            )
            self.patch_adapt = PatchPonderPAM(cfg=cfg)

            # patch_adapt を使う場合、元FeatureExtractorの「前半(pre_extractor + extractors[0])」は未使用になる。
            # そのため、無駄な trainable 扱いを避ける目的で requires_grad を False にしておく。
            # （あなたの状況４（１）と同じ“optimizerに入ってないのにtrainable扱い”問題を避ける思想）
            for p in self.pre_extractor.parameters():
                p.requires_grad = False
            for p in self.extractors[0].parameters():
                p.requires_grad = False

    def forward(self, local_conditions, timesteps=None, context=None):
        """
        Args:
            local_conditions: (B,21,512,512)
            timesteps: (B,) long (PAM使用時のみ必須)
            context: (B,seq,768) (PAM使用時のみ必須)

        Returns:
            output_features: list of 4 tensors
              [
                (B,192,64,64),
                (B,256,32,32),
                (B,384,16,16),
        ...
        if (self.patch_adapt is None) or (not self.use_patch_adaptation):
            # ===== 元の挙動（完全維持）=====
        ...
        # ===== PAMモード =====
        if timesteps is None or context is None:
            raise ValueError("PAM is enabled but timesteps/context is None. "
                             "LocalAdapter.forward must pass timesteps/context to FeatureExtractor.")

        # PAMで (B,192,64,64) の chimera を作る（time/text aware）
        chimera64 = self.patch_adapt(
            local_conditions=local_conditions,
            timesteps=timesteps,
            context=context,
            base_feature_extractor=self,  # 初回だけ stem 初期化に使う（内部で一回だけ）
        )


        # 既存の zero_conv[0] を使って注入特徴へ（既存学習済の表現を活かす）
        # 既存の zero_conv[0] を使って注入特徴へ（既存学習済の表現を活かす）
        output_features.append(self.zero_convs[0](chimera64))


        # FeatureExtractor 後半（extractors[1..]）は“元の層”をそのまま通す
        local_features = chimera64
        for idx in range(1, len(self.extractors)):
            local_features = self.extractors[idx](local_features, None)
            output_features.append(self.zero_convs[idx](local_features))
...
class LocalAdapter(nn.Module):
    def __init__(
        ...
        # ===== FeatureExtractor（PAM対応）=====
        self.feature_extractor = FeatureExtractor(
            local_channels=local_channels,
            inject_channels=inject_channels,
            use_patch_adaptation=use_patch_adaptation,
            pam_cfg=pam_cfg,
        )
    ...
    def forward(self, x, timesteps, context, local_conditions, **kwargs):
        ...
        # ★修正: timesteps/context を FeatureExtractor に渡す
# ここから置き換えてください（models/local_adapter.py の LocalAdapter.forward 内）

        local_features = self.feature_extractor(
            local_conditions,
            timesteps=timesteps,
            context=context,
        )

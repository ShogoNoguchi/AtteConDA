...
class UniControlNet(LatentDiffusion):
    ...
    def configure_optimizers(self):
        ...
        if self.mode in ['local', 'uni']:
            params += [p for p in self.local_adapter.parameters() if p.requires_grad]

        if self.mode in ['global', 'uni']:
            params += [p for p in self.global_adapter.parameters() if p.requires_grad]

        if not self.sd_locked:
            params += [p for p in self.model.diffusion_model.output_blocks.parameters() if p.requires_grad]
            params += [p for p in self.model.diffusion_model.out.parameters() if p.requires_grad]

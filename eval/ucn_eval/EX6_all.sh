#!/usr/bin/env bash
# /home/shogo/coding/eval/ucn_eval/EX6_all.sh
set -euo pipefail

# ==========  ==========
# 生成画像: 90Kckpt + Qwen プロンプト → /data/coding/datasets/WaymoV2/Ucn_byCustomPamModeling_Finetune_6000Step_PretrainIsPureFine60K
# 評価キャッシュ: /data/ucn_eval_cache_ex6

# 0) キャッシュ
mkdir -p /data/ucn_eval_cache_ex6 /data/ucn_eval_cache_ex6/torch_hub
...
docker run --rm --gpus all \
  -e MPLBACKEND=Agg \
  -e PIP_OVERLAY_DIR=/data/ucn_eval_cache_ex6/pip-overlay \
  -e REQS_OVERLAY_PATH=/data/ucn_eval_cache_ex6/requirements.overlay.txt \
  ...
  -v /data/ucn_eval_cache_ex6/torch_hub:/root/.cache/torch/hub \
  ucn-eval \
  --cache-root /data/ucn_eval_cache_ex6 \
  ...
  --gen-root  /data/coding/datasets/WaymoV2/Ucn_byCustomPamModeling_Finetune_60KStep_PretrainIsPureFine60K \
  --prompt-root /data/syndiff_prompts/prompts_eval_waymo \
  --annotation-mode all \
  --annotate-limit 24 \
  --annotate-out /data/ucn_eval_cache_ex6/viz_ex6 \
  --tb --tb-dir /data/ucn_eval_cache_ex6/tensorboard_ex6 \
  --experiment-id EX6 \
